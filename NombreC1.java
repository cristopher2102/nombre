...
public class NombreC1 implements GLEventListener, KeyListener {
    ...
    public static void main(String[] args) {
        ...
        canvas.addGLEventListener(new NombreC1());
        canvas.addKeyListener(new NombreC1());
        frame.add(canvas);
    ...
    public void display(GLAutoDrawable drawable) {
        ...
        glu.gluLookAt(tx, ty, tz,
                tx + Math.cos(Math.toRadians(angle)),
                ty,
                tz + Math.sin(Math.toRadians(angle)),
                0, 1, 0);
    ...
    public void keyTyped(KeyEvent e) {
     ...
     public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_A) {
            angle -= 1; // Gira hacia la izquierda
        }
        if (e.getKeyCode() == KeyEvent.VK_D) {
            angle += 1; // Gira hacia la derecha
        }
        ...
        if (e.getKeyCode() == KeyEvent.VK_UP) {
            tx += .1 * Math.cos(Math.toRadians(angle));
            tz += .1 * Math.sin(Math.toRadians(angle));
        }
        ...
        if (e.getKeyCode() == KeyEvent.VK_ESCAPE) {
            tx = ty = tz = 0;
            rx = ry = rz = 0;
            angle = 0; // Resetea el ángulo de la cámara
        }
    }
